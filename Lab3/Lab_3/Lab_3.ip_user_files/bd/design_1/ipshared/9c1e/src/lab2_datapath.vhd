...
entity lab2_datapath is
    Port(
	       ...
	       risingFallingEdge:  in    STD_LOGIC;
	       triggerTime:        in    STD_LOGIC_VECTOR(9 downto 0):= "0101000000";
	       triggerVolt:        in    STD_LOGIC_VECTOR(9 downto 0):= "0011011100");
end lab2_datapath;

architecture Behavioral of lab2_datapath is

    signal trigger_time_S: UNSIGNED(9 DOWNTO 0);
    signal trigger_volt_S: UNsiGNED(9 DOWNTO 0);
...
-- flag register signals	
    ...
    --signals coming from microblaze
--    signal exTriggerTime_S: STD_LOGIC_VECTOR(9 downto 0);
--    signal exTriggerVolt_S: STD_LOGIC_VECTOR(9 downto 0);
...
begin
    ...
    Lbus_out <= STD_LOGIC_VECTOR(L_bus_unsigned(17 downto 2));
    Rbus_out <= STD_LOGIC_VECTOR(R_bus_unsigned(17 downto 2));
...
--    sw(1) <= '1' when ( (((L_bus_unsigned(17 downto 8)-292) < trigger_volt_S) and ((previous_L_bus_unsigned(17 downto 8) - 292) > trigger_volt_S)) AND (risingFallingEdge = '0')) or
--                ( (((L_bus_unsigned(17 downto 8)-292) > trigger_volt_S) and ((previous_L_bus_unsigned(17 downto 8) - 292) < trigger_volt_S)) AND (risingFallingEdge = '1')) else '0';
    sw(0) <= ready_S;
    
    -- Send the Q_flag signal to the flagQ port out to my_oscope_ip
    flagQ <= Q_flag;
    
    --assign signal to value    
...
--			button_activity <= (old_button xor btn) and btn;
			if(reset_n = '0') then
                ...
                trigger_time_S <= unsigned(triggerTime); -- take trigger value from MicroBlaze
                trigger_volt_S <= unsigned(triggerVolt); -- take trigger value from MicroBlaze
--			--Reset trigger
