...
entity lab2_datapath is
    Port(
           ...
           exTriggerTime:      in    STD_LOGIC_VECTOR(9 downto 0);
           exTriggerVolt:      in    STD_LOGIC_VECTOR(9 downto 0);
...
process(clk)
	begin
		if (rising_edge(clk)) then
			...
			button_activity <= (old_button xor btn) and btn;
			if (exSel = '1') then
                trigger_time_S <= unsigned(exTriggerTime); -- take trigger value from MicroBlaze
                trigger_volt_S <= unsigned(exTriggerVolt); -- take trigger value from MicroBlaze
			--Reset trigger
